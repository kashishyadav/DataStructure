...
public class subtreeOfTree {
    ...
    public static void main(String str[])
    {
       ...
       System.out.println("---------check subtree----------------\n"+chekSubtree(root[0],root[1])+" ");
        
    }
    public static boolean chekSubtree(Node m,Node s)
    {
        if(s==null)
            return true;
        if(m==null)
            return false;
        if(checkIdentical(m,s))
            return true;
        return (chekSubtree(m.left, s)||chekSubtree(m.right, s));
    }
